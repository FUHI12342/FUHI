...
生成(generate_draft)は自動で何度でも安全。配信(publish_draft / retry_unfinished)は
店長の承認操作からのみ呼ばれ、無人で確定投稿されることはない。
"""
import logging
...
MANUAL_DETAIL = 'API未設定のため、文面と画像をコピーして手動投稿してください。'


# --- 文面 -----------------------------------------------------------------

def _arrivals_for(store, date):
    """入荷情報。inventory アプリが有効な場合のみ差し込む(sns は inventory に必須依存しない)。"""
...
def build_body(store, date, cast_shifts, arrivals):
    ...
    casts_section = ''
    if casts:
        lines = '\n'.join(f'・{shift.staff.name}({shift.start_time:%H:%M}〜)' for shift in cast_shifts)
        casts_section = f'\n本日の出勤\n{lines}\n\n'
...
def generate_draft(store, date):
    ...
    body = build_body(store, date, cast_shifts, _arrivals_for(store, date))
    draft, _created = PostDraft.objects.update_or_create(store=store, date=date, defaults={'body': body})
...
def _public_image_url(draft):
    """画像の公開URL(Instagram は公開URL必須)。MEDIA が公開ホスティングされていなければ None。"""
    base = settings.PUBLIC_MEDIA_BASE_URL
    if draft.image and base:
        return base.rstrip('/') + draft.image.url
    return None
...
def _publish_one(draft, adapter, image_url):
    if not adapter.is_configured():
        return PostResult.objects.create(
            draft=draft, platform=adapter.platform, status=PostResult.STATUS_MANUAL, detail=MANUAL_DETAIL,
        )
    try:
        url = adapter.publish(draft.body, image_url)
    except Exception as e:
        # AdapterError に限定しない: 生のネットワーク例外等で配信ループ全体が死ぬと、
        # 残りのプラットフォームが未記録のまま再試行不能になる。
...
def publish_draft(draft, user, adapters=None):
    ...
    - API失敗 → failed(承認は維持。retry_unfinished か手動投稿)
    """
    draft.approve(user)
    image_url = _public_image_url(draft)
    return [_publish_one(draft, adapter, image_url) for adapter in adapters or get_adapters()]
